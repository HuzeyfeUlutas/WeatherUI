    ...
    <main className="min-h-screen overflow-hidden bg-[var(--color-bg)] text-[var(--color-text)]">
      <div className="relative min-h-screen bg-[var(--color-bg)]">
        <header className="sticky left-0 right-0 top-0 z-30 border-b border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-surface)_88%,transparent)] px-4 backdrop-blur-xl md:px-8">
          <div className="mx-auto grid min-h-16 max-w-[1760px] grid-cols-[minmax(0,1fr)_auto] items-center gap-2 py-2 md:flex md:h-16 md:justify-between md:gap-4 md:py-0">
          <div className="flex min-w-0 items-center gap-3 md:gap-7">
            <div>
              <p className="truncate whitespace-nowrap text-base font-semibold tracking-normal text-[var(--color-text)] md:text-xl">
                {t('app.name')}
              </p>
            </div>
          </div>
          ...
          {actions && (
            <div className="col-span-2 flex min-w-0 items-center border-t border-[var(--color-border)] pt-2 md:hidden">
              {actions}
            </div>
          )}
          </div>
